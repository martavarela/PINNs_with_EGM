...
class system_dynamics():
    ...
    def generate_data(self, file_name, dim):
        ...
        Vsav = np.transpose(Vsav,(2, 0, 1))
        Wsav = np.transpose(Vsav,(2, 0, 1))
        V = Vsav.reshape(-1, 1)
        W = Wsav.reshape(-1, 1)

        phie = phie.reshape(-1,1) 
        self.elecpos = elecpos
        self.nelec = np.shape(elecpos)[1]
        if dim == 1:     
            return np.hstack((X, T)), V, W
        return np.hstack((X, Y, T)), V, W, phie, elecpos
    ...
    def pde_2D(self, x, y):
        ...
        #phie = self.integrate(V,self.elecpos)
        phie = self.simple_integrate(V)
        eq_c = phie-y[:,2:3]
    ...
    def integrate(self,v_pred,elecpos):
        #v_pred = v_pred.view(-1, self.max_x, self.max_y)

        #v_pred = v_pred.reshape([int(self.max_t), int(self.max_x), int(self.max_y)])
        ...
        [xel,yel] = np.meshgrid(elecpos[0,:],elecpos[1,:], indexing = 'ij')

        tt = 0
        phie_pred = np.zeros([np.shape(elecpos)[1],tend])
        ...
        for t in range(tend):
            ...
            for k in range(np.shape(elecpos)[1]):
                matx, maty = np.meshgrid((np.arange(2, X) - elecpos[0][k]), (np.arange(2, Y) - elecpos[1][k]))
        ...
        # PLOTTING
        # num_electrodes = phie_pred.shape[0]
        # fig, axes = plt.subplots(num_electrodes, 1, figsize=(10, 6 * num_electrodes))
        # if num_electrodes == 1:
        #     axes = [axes]
        # for k in range(num_electrodes):
        #     axes[k].plot(np.arange(1, tt + 1), phie_pred[k, :], label=f"Electrode {k+1} ({elecpos[0, k]}, {elecpos[1, k]})")
        #     axes[k].set_xlabel('Time (au)')
        #     axes[k].set_ylabel('phie (au)')
        #     axes[k].set_title(f'phie Signal for Electrode {k+1}')
        #     axes[k].legend()
        #     axes[k].grid(True)
        # plt.tight_layout()
        # plt.show()
