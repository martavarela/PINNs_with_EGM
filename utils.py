...
from deepxde.backend import torch as tf
import numpy as np
...
class system_dynamics():
    ...
    def generate_data(self, file_name, dim):
        ...
        phie_norm = (phie - np.mean(phie, axis=1, keepdims=True)) / np.std(phie, axis=1, keepdims=True)
    ...
    def params_to_inverse(self,args_param):
        ...
        if 'a' in args_param:
            ...
            self.a = tf.exp(tf.nn.Parameter(tf.tensor(-3.92, requires_grad=True)))
