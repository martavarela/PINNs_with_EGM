...
class system_dynamics():
    ...
    def calculate_egm(self,v_pred,elecpos):
        v_pred = v_pred.reshape(self.max_y, self.max_x, self.max_t).swapaxes(0, 1)
        X, Y, tend = self.max_x, self.max_y, self.max_t
        ...
        [Dx,Dy] = np.gradient(D_array,self.h,self.h)
        ...
        for t in range(tend):
            ...
            for k in range(np.shape(elecpos)[1]):
                ...
                sum = np.trapz(np.trapz(du[1:-1,1:-1]/distance,axis=0),axis=0)
                phie_pred[k,tt-1] = -sum

            #plotting
        plt.figure(figsize=(10,6))
        for k in range(phie_pred.shape[0]):
            plt.plot(np.arange(1, tt+1), phie_pred[k, :], label=f"Electrode {k+1} ({elecpos[0, k]}, {elecpos[1, k]})")

        plt.xlabel('Time (au)')
        plt.ylabel('phie (au)')
        plt.title('phie Signal for Each Electrode')
        plt.legend()
        plt.grid(True)
        plt.show()
