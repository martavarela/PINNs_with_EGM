...
class PINN():
    ...
    def stable_init(self):
        ...
        losshistory, _ = self.model.train(iterations=1)
    ...
    def train(self, out_path, params):
        ...
        print('----------------Stabilising Weight Initialisation-----------------')
        self.stable_init()
    ...
    def train_1_phase(self, out_path, params):
        if self.inverse:
            ...
            losshistory, train_state = self.model.train(iterations=self.epochs_main, model_save_path = out_path, callbacks=[variable])
        else:
            losshistory, train_state = self.model.train(iterations=self.epochs_main, model_save_path = out_path)
    ...
    def train_3_phase(self, out_path, params):
        ...
        if self.inverse:
            ...
            losshistory, train_state = self.model.train(iterations=self.epochs_init, model_save_path = out_path, callbacks=[variable])
            ## Main phase
            self.model.compile("adam", lr=self.lr)
            losshistory, train_state = self.model.train(iterations=self.epochs_main, model_save_path = out_path, callbacks=[variable])
            ## Final phase
            ...
            losshistory, train_state = self.model.train(iterations=self.epochs_init, model_save_path = out_path)
            ## Main phase
            self.model.compile("adam", lr=self.lr)
            print('-----------------Main Training Phase-----------------')
            losshistory, train_state = self.model.train(iterations=self.epochs_main, model_save_path = out_path)
            ## Final phase
            self.model.compile("L-BFGS-B") # second-order optimisation
