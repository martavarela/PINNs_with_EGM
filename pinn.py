...
class PINN():
    ...
    def __init__(self, dynamics, dim, heter, inverse):
        ...
        self.epochs_main = 1 #1000 # number of epochs for main training phase
    ...
    def num_integration(self,v_pred,obs):
        ...
        # steps = int(v_pred.size/(int(self.dynamics.max_x)*int(self.dynamics.max_y)))
        # v_pred = v_pred.reshape(steps, int(self.dynamics.max_x), int(self.dynamics.max_y))
    ...
    def arrange_phie(self,obs, phie_pred):
        ...
        for i in range(self.dynamics.elecpos.shape[0]):
            ...
            mask = np.isclose(obs[:, 0], x) & np.isclose(obs[:, 1], y)
