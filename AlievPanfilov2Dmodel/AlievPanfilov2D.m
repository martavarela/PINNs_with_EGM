...
phie = [];
phiesav=[];
nsps = length(param.elecpos*2);
calcphie = 1; %gathert
showphie = 220;%gathert
kind = 0;
...
for t=dt:dt:tend % for every timestep
    ...
    % Store the current time t in the array
    t_values = [t_values, t];
    ...
    if mod(ind,gathert)==0
        ...
        if flagmovie&&mod(ind,displayt)==0
            ...
            for i=1:1:length(param.elecpos(1,:))
                elecind = elecind+1;
                text(param.elecpos(1,i),param.elecpos(1,i),num2str(elecind))
            end
            xlabel('x')
            ylabel('y')
            set(gca,'FontSize',8)
            ...
            %         plot(max(1,t-showphie+1):t,...
            %             squeeze(phiesav(sel(1,j),sel(2,j),...
            %             max(1,t-showphie+1):t)),'-',...
            %             'LineWidth',1,'Color',col{j})
...
% visualising results
    % figure(1)
    % for i=1:1:length(param.elecpos)
    %     Vwin = Vsav(param.elecpos(1,i),param.elecpos(1,i),:);
    %     subplot(length(param.elecpos(1,:)),1,i)
    %     hold on
    %     plot(squeeze(Vwin),'LineWidth',2)
    %     grid on
    %     xlabel('Time(AU)')
    %     ylabel('V(AU)')
    %     xlim([0 length(squeeze(Vwin))])
    %     title(['V at (',num2str(param.elecpos(1,i)),',',num2str(param.elecpos(1,i)),') with D=',num2str(param.D)])
    %     legend(['V','W'],'outside')
    % 
    % end
    % figure(2)
    ...
    x = 1:param.h:param.X;
    y = 1:param.h:param.Y;
    save('data_for_python.mat','x','y','t','Vsav','Wsav')
